...
* rl-games要求在训练配置中定义的`minibatch_size`是`horizon_length * num_envs`的一个因数。如果不是这样，您可能会看到断言错误`assert(self.batch_size % self.minibatch_size == 0)`。请相应地调整训练配置`yaml`文件中的参数。
...
* 在训练配置`yaml`文件（例如[HumanoidPPO.yaml](../omniisaacgymenvs/cfg/train/HumanoidPPO.yaml)）中，将参数`mixed_precision`设置为`True`应该仅在使用GPU管道时使用。建议在使用CPU管道时将`mixed_precision`设置为`False`，以防止崩溃。
...
* 如果使用多线程环境包装类`VecEnvMT`运行，您可能会看到类似`Getting states: timeout occurred.`的超时错误。如果您的环境遇到此错误，请尝试增加`VecEnvMT`中的超时变量，该变量可以作为参数在`initialize()`调用时传递。也可以先使用单线程类进行调试，以解决任何错误。
